...
const STATE = {
    ...
    viewDate: new Date(),
    cache: {
        allShifts: [],    // VŠECHNA DATA SMĚN (Cached)
        users: [],
        history: null
    },
...
// =================================================================
// 2. SERVER COMMUNICATION (FETCH API)
// =================================================================
...
async function handleLogin() {
    ...
    if (!pin) return showToast('Zadejte PIN', true);
    if (!isAdminLogin && !name) return showToast('Vyberte jméno', true);
...
function openDayModal(date, shifts, confirmedShift) {
    ...
    const czDate = date.getDate() + '. ' + (date.getMonth() + 1) + '.';
...
function renderBrigadeerStats() {
    ...
    document.getElementById('b-to-pay').textContent = Math.round(Math.max(0, totalEarned - totalPaid)) + ' Kč';
    document.getElementById('b-worked-hours').textContent = workedH.toFixed(1) + ' h';
}
